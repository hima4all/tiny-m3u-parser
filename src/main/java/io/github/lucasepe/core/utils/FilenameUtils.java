...
public class FilenameUtils {
    ...
    public static boolean wildcardMatchOnSystem(final String filename, final String wildcardMatcher) {
        return wildcardMatch(filename, wildcardMatcher, !isSystemWindows());
    }
            ...
            while (wcsIdx < wcs.length) {
                ...
                if (wcs[wcsIdx].equals("?")) {
                    ...
                    if (anyChars) {
                        ...
                        textIdx = checkIndexOf(filename, textIdx, wcs[wcsIdx], !caseSensitivity);
                        if (textIdx == NOT_FOUND) {
                            // token not found
                            break;
                        }
                        final int repeat = checkIndexOf(filename, textIdx + 1, wcs[wcsIdx], !caseSensitivity);
                        if (repeat >= 0) {
                            backtrack.push(new int[] {wcsIdx, repeat});
                        }
                    } else {
                        // matching from current position
                        if (!checkRegionMatches(filename, textIdx, wcs[wcsIdx], !caseSensitivity)) {
                            // couldnt match token
                            break;
                        }
                    }
     ...
     * @since 2.0
     */
    private static int checkIndexOf(final String str,
        ...
        if (endIndex >= strStartIndex) {
            for (int i = strStartIndex; i <= endIndex; i++) {
                if (checkRegionMatches(str, i, search, ignoreCase)) {
                    return i;
                }
            }
        }
     ...
     * @throws NullPointerException if either string is null
     */
    private static boolean checkRegionMatches(final String str,
                                              final int strStartIndex,
                                              final String search,
                                              final boolean ignoreCase) {
